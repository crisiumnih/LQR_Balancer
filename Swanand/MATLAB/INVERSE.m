...
% State-space matrices for the linearized system
A = [0, 0, 1, 0;
     0, 0, 0, 1;
     0, -(m*g)/M, 0, 0;
     0, (M+m)*g/(M*l), 0, 0];
...
Q = diag([100, 10000, 1, 1]); % State error weightings
R = 0.001; % Control effort weighting

% Compute the LQR controller gain matrix K
K = lqr(A, B, Q, R);

% Initial state [x; theta; dx; dtheta]
x0 = [0; pi; 0; 0]; % Small initial angle, near upright position

% Desired end state
target_position = -1.0; % meters (cart position)
...
function dxdt = pendulum_dynamics(t, x, A, B, K, target_position, target_angle)
    ...
    ref = [target_position; target_angle; 0; 0]; % Desired state [x; theta; dx; dtheta]
    u = -K * (x - ref); % LQR control law
    dxdt = A * x + B * u; % State derivative
end

% Simulation time
t = 0:0.01:100; % Time vector from 0 to 10 seconds with 0.01s step
